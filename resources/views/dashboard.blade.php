            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form action="{{ route('dashboard.store') }}" method="post">
                        ...
                        <div class="flex">
                            <button type='submit' class="bg-green-500 font-medium text-sm text-white mx-2 py-1 px-2 w-28 rounded-full">
                                {{__('Create')}}
                            </button>
                        </div>
                    </form>
                            ...
                            @foreach ($gh_names as $gh_name)
                                <li class="flex justify-between">
                                    ...
                                    <form action="{{ route('dashboard.destroy', $gh_name[0]->id) }}" method="POST"
                                        ...
                                        @method('delete')
                                        @csrf
                                        <button type="submit" class="bg-red-500 font-medium text-sm text-white mx-2 py-1 px-2 w-28 rounded-full">
                                            {{__('Delete')}}
                                        </button>
                                    </form>
                                </li>
